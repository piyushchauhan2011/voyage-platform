...
GitHub Actions (`.github/workflows/ci.yml`) runs these on every push/PR to `main`: parallel **quality**, **unit-tests** (+ JaCoCo), and **integration-tests** (Docker/Testcontainers), then **package**. On pull requests, the unit-tests job also posts a sticky **coverage comment** (overall + changed-line %) and **annotates uncovered changed lines** on the Files tab; the hard gate remains JaCoCo ≥40% lines.
