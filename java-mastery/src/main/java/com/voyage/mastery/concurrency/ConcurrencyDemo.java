...
/**
 * Multithreading primitives — Thread, pools, futures, locks, volatile, atomics.
 * See DoubleBookingDemo for the senior "1000 users, one room" race.
 */
public class ConcurrencyDemo {
    ...
    private static void threadAndRunnableDemo() {
        ...
        Thread t = new Thread(() -> log.add("booked-by-" + Thread.currentThread().getName()), "booking-worker");
        t.start();
        try {
    ...
    private static void volatileDemo() {
        ...
        System.out.println("Without volatile, HotSpot may cache the flag in a register → infinite spin");
    }
