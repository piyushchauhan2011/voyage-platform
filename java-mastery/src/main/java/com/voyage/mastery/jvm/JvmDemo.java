 ...
 *
 * <p>Pipeline: .java source → javac → .class bytecode → JVM (class loader + interpreter/JIT) →
 * machine code
 *
 * <p>Memory regions the JVM manages: Heap — objects / arrays (shared across threads); subject to GC
 * Stack — per-thread frames: locals, operand stack, return address (method calls) Metaspace — class
 * metadata, method bytecode, constant pools (native memory, not the Java heap)
 */
public class JvmDemo {
  ...
  private static void heapGenerationsDemo() {
    ...
    System.out.println(
        "Minor GC clears Eden; survivors promote or flip to the other Survivor space");
  }
  ...
  private static void allocationChurnDemo() {
    ...
    System.out.printf(
        "Free after  churn: %d KB (GC may reclaim asynchronously)%n", freeAfter / 1024);
  }

  private static void gcFlagsCheatSheet() {
    System.out.println("\n-- GC / memory flags (cheat sheet) --");
    System.out.println(
        "  -Xms512m -Xmx512m          initial & max heap (pin size to avoid resize)");
