...
/**
 * Phase 8 knobs, read from the {@code application.ai.*} namespace to match the
 * convention the other phases use ({@code application.kafka.*}, {@code application.redis.*}).
 */
@Component
public class AiLabProperties {
    ...
    public AiLabProperties(
            ...
            @Value("${spring.ai.google.genai.embedding.text.model:text-embedding-004}") String embeddingModel,
    ...
    /**
     * The app is allowed to start without a Gemini key so the Kafka/Redis/JPA phases keep
     * working. Every AI endpoint checks this first and fails with a readable message rather
     * than letting a 401 from Google surface as a stack trace.
     */
