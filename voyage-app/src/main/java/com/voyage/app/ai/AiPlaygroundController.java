 ...
 *
 * Mirrors the RabbitMQ/JPA/Postgres playgrounds: thin controller, logic in the services,
 * local exception handlers so failures come back as readable JSON.
 */
@RestController
...
public class AiPlaygroundController {
    ...
    public AiPlaygroundController(AiLabProperties properties,
    ...
    @PostMapping("/embed")
    public AiPlaygroundService.EmbeddingResult embed(@RequestBody(required = false) EmbedRequest request) {
        return aiPlaygroundService.embed(
                request == null ? null : request.first(),
                request == null ? null : request.second());
    }
    ...
    @PostMapping("/search")
    public VectorSearchService.SearchResult search(@RequestBody(required = false) SearchRequestBody request) {
    ...
    public ToolCatalogResult tools() {
        List<ToolDescription> tools = List.of(
                new ToolDescription("searchHotels",
                        "Search hotels by city and/or nightly price range",
                        List.of("city (optional)", "minPrice (optional)", "maxPrice (optional)")),
                new ToolDescription("getHotelDetails",
                        "Fetch one hotel's full details by id",
                        List.of("hotelId")),
                new ToolDescription("checkAvailability",
                        "Check room availability for a date range",
                        List.of("hotelId", "checkInDate", "checkOutDate", "roomType (optional)")));
        return new ToolCatalogResult(tools,
    ...
    public HotelAssistantService.AssistantResult assistant(@RequestBody(required = false) AssistantRequest request) {
    ...
    public HotelAssistantService.ClearResult clearConversation(
        ...
        return hotelAssistantService.clearConversation(request == null ? null : request.conversationId());
    }
