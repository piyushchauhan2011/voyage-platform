 ...
 *
 * <p>Everything here is deliberately transparent — each result carries the prompt that was actually
 * sent and an observation explaining what to notice, because the interesting part of learning AI
 * plumbing is seeing the intermediate state, not just the final answer.
 */
@Service
...
public class AiPlaygroundService {
  ...
  /**
   * Only the leading slice of each vector is returned to the browser. A full text-embedding-004
   * vector is 768 floats, which is noise on screen.
   */
  ...
  /**
   * The smallest useful Spring AI call: a system message setting the persona and a user message. No
   * retrieval, no tools — so any hotel it names is invented. That failure is the point, and it is
   * what the RAG rung fixes.
   */
  public ChatResult chat(String message, String systemPrompt) {
    ...
    ChatResponse response =
        chatClient.prompt().system(system).user(userMessage).call().chatResponse();
  ...
  /**
   * Prompt templates are string interpolation with guardrails: placeholders are declared up front,
   * so the caller supplies data rather than concatenating raw text into the prompt.
   */
  public PromptTemplateResult promptTemplate(String city, Double budget, String vibe) {
    ...
    String rendered =
        PromptTemplate.builder().template(template).variables(variables).build().render();

    long startedAt = System.currentTimeMillis();
    ChatResponse response = chatClient.prompt().user(rendered).call().chatResponse();
    long tookMs = System.currentTimeMillis() - startedAt;
   ...
   *
   * <p>Cosine similarity is the entire trick behind semantic search: text with related meaning
  ...
  public EmbeddingResult embed(String first, String second) {
    properties.requireApiKey();
    String left =
        (first == null || first.isBlank()) ? "a quiet room a short walk from the sand" : first;
    ...
    return new EmbeddingResult(
        ...
        cosineSimilarity(
            leftVector, embeddingModel.embed("annual report on quarterly tax filings")),
        tookMs,
        ...
        "Every vector here is "
            + leftVector.length
            + " floats — which is exactly why "
            + "spring.ai.vectorstore.pgvector.dimensions has to be set to match.");
  }

  /**
   * Standard cosine similarity. Written out rather than pulled from a library because seeing the
   * formula is half the lesson: it is just a normalised dot product.
   */
