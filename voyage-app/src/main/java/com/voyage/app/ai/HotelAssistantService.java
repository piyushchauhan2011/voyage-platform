 ...
 *
 * "Find hotels near beach under $100" is really two different questions wearing one coat:
 *
 *   "near beach"  — fuzzy, subjective, answered by embedding similarity over descriptions
 *   "under $100"  — exact, arithmetic, answered by a SQL query through a tool call
 *
 * Neither mechanism can do the other's job. Similarity search cannot compare numbers, and a
 * SQL WHERE clause cannot tell that "sand and surf" means beach. The agent's value is deciding
 * which to reach for, and combining the results.
 *
 * The response carries a full trace — retrieved documents, tools invoked, token usage — because
...
public class HotelAssistantService {
    ...
    public HotelAssistantService(ChatClient.Builder chatClientBuilder,
    ...
    public ClearResult clearConversation(String conversationId) {
        ...
        return new ClearResult(conversation, "Conversation history cleared. The next question starts fresh.");
    }
    ...
    private static List<VectorSearchService.Match> retrievedDocuments(ChatClientResponse response) {
        ...
        if (documents instanceof List<?> list) {
            return ((List<Document>) list).stream()
                    .map(document -> new VectorSearchService.Match(
                            document.getId(),
