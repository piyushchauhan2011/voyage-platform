...
public class HotelCatalogSeeder {
  ...
  public HotelCatalogSeeder(
      HotelRepository hotelRepository, RoomInventoryRepository roomInventoryRepository) {
  ...
  public SeedResult seed() {
    ...
    List<Hotel> toCreate =
        catalog.stream().filter(hotel -> !existingNames.contains(hotel.getName())).toList();
  ...
  /**
   * Gives every seeded hotel a rolling window of availability so the checkAvailability tool has
   * real rows to read instead of always answering "unknown".
   */
