 ...
 *
 * <p>Two details worth understanding:
 *
 * <p>1. Document IDs are derived deterministically from the hotel ID, so re-running ingestion
 ...
 *
 * <p>2. Structured fields (city, price) go into metadata rather than the embedded text. Metadata is
 * filterable with exact SQL-like predicates, whereas embedded text is only ever matched fuzzily.
 * Prices in particular must never be left to similarity search — "under $100" is an arithmetic
 * question, and that is what the tool-calling rung is for.
 */
@Service
...
public class HotelDocumentIngestor {
  ...
  public HotelDocumentIngestor(
      HotelRepository hotelRepository, VectorStore vectorStore, AiLabProperties properties) {
  ...
  /**
   * Name and city are folded into the embedded text on purpose: a query naming a city should still
   * rank that city's hotels highly even when no metadata filter is supplied.
   *
   * <p>The price appears in both the text and the metadata, for different reasons. Only the text
   * reaches the model — QuestionAnswerAdvisor pastes the document body into the prompt and leaves
   * metadata behind — so without it the RAG rung cannot answer a budget question at all. The
   * metadata copy is what filter expressions compare against.
   *
   * <p>Having the price in the text does not make similarity search able to do arithmetic: the
   * model reads "$74 per night" as prose and reasons about it approximately. That is the gap the
   * tool-calling rung closes.
   */
    ...
    String text =
        """
            ...
            .formatted(
                ...
                hotel.getAmenities() == null ? "not listed" : hotel.getAmenities())
            .strip();
  ...
  static String documentId(Long hotelId) {
    return UUID.nameUUIDFromBytes((DOCUMENT_ID_PREFIX + hotelId).getBytes(StandardCharsets.UTF_8))
        .toString();
  }
