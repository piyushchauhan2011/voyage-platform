...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiError> handleBookingNotAvailable(
      BookingNotAvailableException exception, HttpServletRequest request) {
    return build(HttpStatus.CONFLICT, exception.getMessage(), request);
  }

  @ExceptionHandler(PaymentFailedException.class)
  public ResponseEntity<ApiError> handlePaymentFailed(
      PaymentFailedException exception, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiError> handleValidation(
    ...
    List<ApiError.FieldError> fieldErrors =
        exception.getBindingResult().getFieldErrors().stream().map(this::toApiFieldError).toList();
    ApiError apiError =
        ApiError.validation(
  ...
  public ResponseEntity<ApiError> handleUnexpected(
    ...
    log.error(
        "Unhandled exception for {} {}", request.getMethod(), request.getServletPath(), exception);
  ...
  private ResponseEntity<ApiError> build(
    ...
    return ResponseEntity.status(status)
        .body(
            ApiError.of(
                status.value(), status.getReasonPhrase(), message, request.getServletPath()));
  }
