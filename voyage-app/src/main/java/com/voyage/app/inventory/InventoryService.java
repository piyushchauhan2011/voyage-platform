...
public class InventoryService {
  ...
  @Transactional
  public RoomInventory createInventory(
      Long hotelId, RoomType roomType, LocalDate date, int availableRooms) {
    Hotel hotel =
        hotelRepository
            .findById(hotelId)
  ...
  public List<InventoryResponse> findInventoryResponses(
      Long hotelId, LocalDate from, LocalDate to, RoomType roomType) {
  ...
  private void adjustInventory(Long hotelId, RoomType roomType, LocalDate stayDate, int delta) {
    RoomInventory inventory =
        roomInventoryRepository
            .findForUpdate(hotelId, stayDate, roomType)
            .orElseThrow(
                () ->
                    new BookingNotAvailableException(
                        "Inventory not configured for %s on %s".formatted(roomType, stayDate)));
    if (delta < 0 && inventory.getAvailableRooms() <= 0) {
      throw new BookingNotAvailableException(
          "No rooms available for %s on %s".formatted(roomType, stayDate));
    }
  ...
  public RoomInventory updateAvailability(Long inventoryId, int availableRooms) {
    RoomInventory inventory =
        roomInventoryRepository
            ...
            .orElseThrow(
                () -> new ResourceNotFoundException("Inventory not found: " + inventoryId));
