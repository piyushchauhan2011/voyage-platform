...
public class DeadLetterHotelEventService {
    ...
    public DeadLetterHotelEvent retryDeadLetter(Long id, String payloadOverride) {
        DeadLetterHotelEvent event = deadLetterHotelEventRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Dead-letter event not found: " + id));

        String payloadToReplay = StringUtils.hasText(payloadOverride) ? payloadOverride : event.getPayload();
    ...
    public void markResolvedForEventId(String originalEventId) {
        ...
        for (DeadLetterHotelEvent event : deadLetterHotelEventRepository.findByOriginalEventIdOrderByDeadLetteredAtDesc(originalEventId)) {
