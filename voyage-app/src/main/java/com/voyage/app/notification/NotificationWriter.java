...
public class NotificationWriter {
    ...
    public NotificationWriter(NotificationRepository notificationRepository, UserRepository userRepository) {
    ...
    public void writeBookingConfirmed(BookingConfirmedEvent event) {
        ...
        String message = "Booking %d confirmed for %s from %s to %s".formatted(
                event.bookingId(),
                event.hotelName(),
                event.checkIn(),
                event.checkOut()
        );
        notificationRepository.save(new Notification(user, event.bookingId(), NotificationType.BOOKING_CONFIRMED, message));
    }
    ...
    public void writeBookingCancelled(BookingCancelledEvent event) {
        User user = userRepository.findById(event.userId())
                .orElseThrow(() -> new ResourceNotFoundException("User not found: " + event.userId()));
        String message = "Booking %d cancelled for %s from %s to %s".formatted(
                event.bookingId(),
                event.hotelName(),
                event.checkIn(),
                event.checkOut()
        );
        notificationRepository.save(new Notification(user, event.bookingId(), NotificationType.BOOKING_CANCELLED, message));
    }
}