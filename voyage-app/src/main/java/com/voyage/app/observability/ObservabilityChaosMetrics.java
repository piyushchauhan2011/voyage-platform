...
/**
 * Custom Micrometer series for the SRE chaos lab. Prometheus recording rules and Grafana panels key
 * off these names.
 */
@Component
public class ObservabilityChaosMetrics {
  ...
  public ObservabilityChaosMetrics(MeterRegistry meterRegistry) {
    this.deadlocksTotal =
        Counter.builder("voyage_chaos_deadlocks_total")
            .description("Lock-contention / deadlock lab runs that completed")
            .register(meterRegistry);
    this.redisEvictionRunsTotal =
        Counter.builder("voyage_chaos_redis_eviction_runs_total")
            .description("Redis eviction fill runs")
            .register(meterRegistry);
    this.kafkaLagRunsTotal =
        Counter.builder("voyage_chaos_kafka_lag_runs_total")
            .description("Kafka lag burst runs")
            .register(meterRegistry);
    this.slowApiInvocationsTotal =
        Counter.builder("voyage_chaos_slow_api_total")
    ...
    Gauge.builder("voyage_chaos_leak_bytes", leakBytesHeld, AtomicLong::get)
        ...
        .register(meterRegistry);
    Gauge.builder(
            "voyage_chaos_kafka_consumer_paused",
            kafkaConsumerPaused,
