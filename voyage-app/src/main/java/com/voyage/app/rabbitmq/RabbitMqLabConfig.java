...
public class RabbitMqLabConfig {
  ...
  public Binding bookingConfirmBinding(
      Queue bookingConfirmQueue, DirectExchange voyageJobsExchange) {
  ...
  public Binding emailSendBinding(Queue emailSendQueue, DirectExchange voyageJobsExchange) {
    return BindingBuilder.bind(emailSendQueue).to(voyageJobsExchange).with(ROUTING_KEY_EMAIL_SEND);
  }
}
