...
public class RabbitMqPlaygroundService {
    ...
    private static final Set<String> KNOWN_ROUTING_KEYS = Set.of(
            RabbitMqLabConfig.ROUTING_KEY_BOOKING_CONFIRM,
    ...
    public SetupResult setup() {
        ...
        amqpAdmin.declareBinding(BindingBuilder.bind(bookingQueue)
                .to(exchange)
                .with(RabbitMqLabConfig.ROUTING_KEY_BOOKING_CONFIRM));
        amqpAdmin.declareBinding(BindingBuilder.bind(emailQueue)
                .to(exchange)
    ...
    public PublishResult publish(String routingKey, String payload) {
        ...
        LabJobMessage job = new LabJobMessage(
        ...
        String observation = known
                ? "Published to exchange '" + exchangeName + "' with routing key '" + trimmedKey + "'."
                : "Published with unbound routing key '" + trimmedKey + "'. RabbitMQ drops it — no queue is bound.";
        String tip = known
    ...
    public RoutingDemoResult routingDemo() {
        ...
        PublishResult matched = publish(
                RabbitMqLabConfig.ROUTING_KEY_BOOKING_CONFIRM,
                "routing-demo-matched"
        );
        ...
        return new RoutingDemoResult(
                matched,
                unmatched,
                "Matching key booking.confirm binds to " + bookingConfirmQueue
    ...
    public record ConsumedResult(
            List<LabJobDelivery> deliveries,
            String observation,
            String tip
    ) {
    }
