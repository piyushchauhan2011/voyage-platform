...
/**
 * Attribute-based access checks for hotels, inventory, bookings, and payments. Roles are coarse
 * gates; this service evaluates resource attributes (ownership, SaaS plan, rate plan).
 */
@Service
public class HotelAccessService {
  ...
  public void assertCanWriteInventory(Long hotelId) {
    ...
    if (!isAdmin(user) && !hotel.getSaasPlan().isInventoryWritesAllowed()) {
      throw new ForbiddenException(
          "Inventory writes require PRO or ENTERPRISE plan (hotel plan is "
              + hotel.getSaasPlan()
              + ")");
    }
  }
  ...
  public void assertCanCreateHotel(User manager) {
    ...
    if (owned >= effectivePlan.getMaxHotels()) {
      throw new ForbiddenException(
          "Hotel limit reached for plan "
              + effectivePlan
              + " (max "
              + effectivePlan.getMaxHotels()
              + ")");
    }
  }
  ...
  public List<Long> managedHotelIds(User user) {
    return hotelRepository.findByManager_Id(user.getId()).stream().map(Hotel::getId).toList();
  }
