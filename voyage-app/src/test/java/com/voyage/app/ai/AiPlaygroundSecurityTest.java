...
/**
 * The AI playground exposes an outbound paid API and can write to the vector store, so it sits
 * behind ADMIN like the other lab playgrounds.
 */
@SpringBootTest
...
class AiPlaygroundSecurityTest {
  ...
  void assistant_withoutToken_returns401() throws Exception {
    mockMvc.perform(post("/api/ai/playground/assistant")).andExpect(status().isUnauthorized());
  }
  ...
  void assistant_withUserRole_returns403() throws Exception {
    mockMvc
        .perform(
            post("/api/ai/playground/assistant")
  ...
  void status_withoutToken_returns401() throws Exception {
    mockMvc.perform(get("/api/ai/playground/status")).andExpect(status().isUnauthorized());
  }
  ...
  void labPageStaysPublic() throws Exception {
    mockMvc.perform(get("/ui/ai")).andExpect(status().isOk());
  }
}
