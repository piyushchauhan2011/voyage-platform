...
/**
 * Proves the real pgvector round trip: schema creation, insert, and cosine ranking, against the
 * same {@code pgvector/pgvector:pg16} image docker-compose uses.
 *
 * <p>The embedding model is a deterministic fake rather than Gemini. That keeps the test free,
 * offline, and repeatable while still exercising everything we actually own — the document shape,
 * the metadata filter, and the idempotent re-ingest. Vector arithmetic itself is Postgres's job and
 * does not need our verification.
 *
 * <p>Prefers Testcontainers. If the Docker API is unreachable (common with some Docker Desktop
 * socket setups, the same reason {@code RabbitMqIntegrationTestSupport} has a fallback), it uses a
 * pgvector instance on {@code localhost:5433}, which you can start with:
 *
 * <pre>{@code
...
class PgVectorIntegrationTest {
  ...
  static void requirePgvector() {
    Assumptions.assumeTrue(
        DOCKER_API_AVAILABLE || FALLBACK_REACHABLE,
        "pgvector tests need Testcontainers Docker or a pgvector instance on localhost:"
            + FALLBACK_PORT);
  }
  ...
  static void properties(DynamicPropertyRegistry registry) {
    ...
    // Satisfies the "is a key present" guard. Nothing here calls Google — the stub models do the
    // work.
  ...
  void setUp() {
    ...
    hotelRepository.save(
        new Hotel(
            ...
            "Lisbon",
            74.0,
            "Budget rooms a barefoot walk from a wide stretch of sand and surf.",
            "free wifi"));
    hotelRepository.save(
        new Hotel(
            "Azure Sands Resort",
            "Nice",
            320.0,
            "Large shorefront resort with private access to the sand.",
            "spa, pools"));
    hotelRepository.save(
        new Hotel(
            "Alpine Ridge Chalet",
            "Innsbruck",
            145.0,
            "Timber chalet at the base of the ski lifts, with a boot drying room.",
            "sauna"));
  }
  ...
  /**
   * Counts rows in the table rather than via similarity search: a document whose similarity to the
   * probe is exactly zero is filtered out even by similarityThresholdAll, which would make a
   * search-based count quietly wrong.
   */
  ...
  static class FakeEmbeddingConfiguration {
    ...
    /**
     * Enabling the AI lab brings up the chat-backed services too, which need a ChatModel. This test
     * is about the vector store, so the model just has to exist.
     */
    @Bean
    @Primary
    ...
    /**
     * Hashes each token into a fixed number of buckets — a bag-of-words vector. Crude next to a
     * real model, but deterministic, offline, and enough that documents sharing vocabulary land
     * near each other.
     */
    @Bean
    @Primary
