...
class BookingCancellationFlowTest {
    ...
    void cancellingBooking_restoresInventoryAndCreatesCancellationNotification() {
        ...
        assertEquals(1, roomInventoryRepository.findByHotelIdAndDateAndRoomType(hotel.getId(), checkIn.plusDays(1), RoomType.SUITE).orElseThrow().getAvailableRooms());
        assertEquals(2, notificationRepository.count());
        assertEquals(NotificationType.BOOKING_CANCELLED, notificationRepository.findAll().stream()
