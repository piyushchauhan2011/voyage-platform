...
class BookingControllerCancellationTest {
  ...
  void setUp() {
    ...
    User user =
        userRepository.save(
            new User(
                "cancel-controller-user",
                "cancel-controller-user@test.com",
  ...
  void cancelBooking_restoresInventory() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/v1/bookings/{id}", bookingId).header("Authorization", userToken))
    ...
    assertEquals(
        ...
        roomInventoryRepository
            .findByHotelIdAndDateAndRoomType(hotel.getId(), checkIn, RoomType.DOUBLE)
            .orElseThrow()
            .getAvailableRooms());
    assertEquals(
        1,
        roomInventoryRepository
            .findByHotelIdAndDateAndRoomType(hotel.getId(), checkIn.plusDays(1), RoomType.DOUBLE)
            .orElseThrow()
            .getAvailableRooms());
  }
}
