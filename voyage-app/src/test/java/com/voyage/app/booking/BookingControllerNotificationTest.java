...
class BookingControllerNotificationTest {
    ...
    void createBooking_createsNotificationAndAllowsMarkRead() throws Exception {
        ...
        String notificationBody = mockMvc.perform(get("/api/v1/notifications/me")
                        .header("Authorization", userToken))
                .andExpect(status().isOk())
        ...
        long notificationId = objectMapper.readTree(notificationBody).get("content").get(0).get("id").asLong();
