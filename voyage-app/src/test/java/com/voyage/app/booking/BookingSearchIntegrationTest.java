...
class BookingSearchIntegrationTest {
    ...
    void jpqlQueriesReturnExpectedBookings() {
        ...
        List<Booking> confirmedTokyoBookings = bookingService.findByHotelAndStatus(tokyo.getId(), BookingStatus.CONFIRMED);
        long overlapCount = bookingService.countByHotelAndDateRange(tokyo.getId(), LocalDate.now().plusDays(5), LocalDate.now().plusDays(7));
    ...
    void specificationsFilterBookings() {
        ...
        List<Booking> result = bookingService.searchWithSpecifications(criteria, PageRequest.of(0, 10)).getContent();
