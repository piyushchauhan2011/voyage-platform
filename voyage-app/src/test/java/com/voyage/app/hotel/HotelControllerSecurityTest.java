...
/**
 * Demonstrates the three security outcomes: 401 Unauthorized — no/invalid token (unauthenticated)
 * 403 Forbidden — valid token but wrong role (authenticated, not authorized) 2xx — valid token with
 * required role @WithMockUser bypasses the JWT filter and sets a pre-built Authentication directly
...
class HotelControllerSecurityTest {
  ...
  void getHotels_withoutToken_returns200() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/hotels")).andExpect(status().isOk());
  }
  ...
  void deleteHotel_withoutToken_returns401() throws Exception {
    mockMvc.perform(delete("/api/v1/hotels/1")).andExpect(status().isUnauthorized());
  }
