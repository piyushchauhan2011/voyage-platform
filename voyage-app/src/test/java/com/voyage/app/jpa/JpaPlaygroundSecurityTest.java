...
class JpaPlaygroundSecurityTest {
  ...
  void seed_withoutToken_returns401() throws Exception {
    mockMvc.perform(post("/api/jpa/playground/seed")).andExpect(status().isUnauthorized());
  }
  ...
  void nplus1_withAdmin_returnsQueryCount() throws Exception {
    ...
    mockMvc
        .perform(post("/api/jpa/playground/seed").header("Authorization", token))
        .andExpect(status().isOk());

    mockMvc
        .perform(post("/api/jpa/playground/loading/nplus1").header("Authorization", token))
        .andExpect(status().isOk())
