...
class ObservabilityPlaygroundSecurityTest {
  ...
  void cpuSpike_withoutToken_returns401() throws Exception {
    mockMvc
        .perform(post("/api/observability/playground/cpu-spike"))
  ...
  void cpuSpike_withUserRole_returns403() throws Exception {
    mockMvc
        .perform(
            post("/api/observability/playground/cpu-spike")
  ...
  void slowApi_withAdminRole_returns200() throws Exception {
    mockMvc
        .perform(
            post("/api/observability/playground/slow-api")
                .param("delayMs", "25")
  ...
  void memoryLeak_andRelease_withAdmin_returns200() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/observability/playground/memory-leak")
                .param("mb", "1")
                .param("holdSeconds", "0")
    ...
    mockMvc
        .perform(delete("/api/observability/playground/memory-leak").header("Authorization", token))
        .andExpect(status().isOk())
  ...
  void kafkaLag_whenKafkaDisabled_returns409() throws Exception {
    mockMvc
        .perform(
            post("/api/observability/playground/kafka-lag")
  ...
  void redisEviction_whenRedisDisabled_returns409() throws Exception {
    mockMvc
        .perform(
            post("/api/observability/playground/redis-eviction")
