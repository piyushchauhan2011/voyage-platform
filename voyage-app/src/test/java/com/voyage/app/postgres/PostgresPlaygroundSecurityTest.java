...
class PostgresPlaygroundSecurityTest {
  ...
  void seed_withoutToken_returns401() throws Exception {
    mockMvc.perform(post("/api/postgres/playground/seed")).andExpect(status().isUnauthorized());
  }
  ...
  void seed_withUserRole_returns403() throws Exception {
    mockMvc
        .perform(
            post("/api/postgres/playground/seed")
  ...
  void explain_withAdmin_returnsPlan() throws Exception {
    ...
    mockMvc
        .perform(post("/api/postgres/playground/seed").header("Authorization", token))
        .andExpect(status().isOk());

    mockMvc
        .perform(
            post("/api/postgres/playground/explain")
