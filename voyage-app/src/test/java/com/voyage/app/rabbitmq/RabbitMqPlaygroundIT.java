...
@ActiveProfiles("test")
class RabbitMqPlaygroundIT extends RabbitMqIntegrationTestSupport {
  ...
  void setUpAdminToken() {
    ...
    User user =
        new User(
            "rmq-admin", "rmq-admin@test.com", passwordEncoder.encode("password123"), Role.ADMIN);
  ...
  void setupPublishAndConsume_roundTrip() throws Exception {
    mockMvc
        .perform(post("/api/rabbitmq/playground/setup").header("Authorization", adminToken))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.exchange").value(org.hamcrest.Matchers.startsWith("voyage.jobs.test.")))
    ...
    MvcResult publishResult =
        mockMvc
            .perform(
                post("/api/rabbitmq/playground/publish")
                    .header("Authorization", adminToken)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(
                        "{\"routingKey\":\"booking.confirm\",\"payload\":\"integration-job\"}"))
            .andExpect(status().isOk())
    ...
    for (int attempt = 0; attempt < 40; attempt++) {
      String body =
          mockMvc
              .perform(get("/api/rabbitmq/playground/consumed").header("Authorization", adminToken))
              .andExpect(status().isOk())
              .andReturn()
              .getResponse()
              .getContentAsString();
  ...
  void routingDemo_andCompare_exposeEducationalFields() throws Exception {
    ...
    mockMvc
        .perform(post("/api/rabbitmq/playground/routing-demo").header("Authorization", adminToken))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(get("/api/rabbitmq/playground/compare").header("Authorization", adminToken))
        .andExpect(status().isOk())
