...
class RedisPlaygroundIntegrationTest extends RedisIntegrationTestSupport {
  ...
  void stringAndTtlEndpoints_roundTrip() throws Exception {
    mockMvc
        .perform(
            post("/api/redis/playground/strings/demo:test")
                .header("Authorization", adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    objectMapper.writeValueAsString(new RedisStringWriteRequest("available", 90L))))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.key").value("demo:test"))
        .andExpect(jsonPath("$.value").value("available"));

    mockMvc
        .perform(get("/api/redis/playground/ttl/demo:test").header("Authorization", adminToken))
        .andExpect(status().isOk())
  ...
  void pubSubAndLockEndpoints_workForAdminUsers() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/redis/playground/pubsub/publish")
                .header("Authorization", adminToken)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    for (int attempt = 0; attempt < 20; attempt++) {
      String response =
          mockMvc
              .perform(
                  get("/api/redis/playground/pubsub/messages").header("Authorization", adminToken))
              .andExpect(status().isOk())
              .andReturn()
              .getResponse()
              .getContentAsString();
    ...
    mockMvc
        .perform(
            post("/api/redis/playground/locks/inventory:hotel-42/acquire")
                .header("Authorization", adminToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    objectMapper.writeValueAsString(new RedisLockAcquireRequest("owner-1", 30L))))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/api/redis/playground/locks/inventory:hotel-42/acquire")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    objectMapper.writeValueAsString(new RedisLockAcquireRequest("owner-2", 30L))))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.acquired").value(false));

    mockMvc
        .perform(
            delete("/api/redis/playground/locks/inventory:hotel-42")
                .header("Authorization", adminToken)
                .param("owner", "owner-1"))
        .andExpect(status().isOk())
